...
public class RewardPopUp : MonoBehaviour
{
    ...
    public event Action OnEndQeueu; //보상 보여준 후 처리 필요하면 구독
    ...
    private void Awake()
    {
        ...
        if (_ItemsSO == null) _ItemsSO = Manager.Item?.ItemImages;
    }
    public void StartShowingReward()
    {
    ...
    private void ShowNext() // 다음 보상 표시
    {
        ...
        else if (MiniGameRewardPicker.TryGetRoomFromRewardType(reward.RewardType, out var room)) // 방 타입이면
        {
            _icon.sprite = _ItemsSO.GetItemSprite(reward.RewardType); //아이템 스프라이트
            _amount.text = $"new Roooooom";
        }
